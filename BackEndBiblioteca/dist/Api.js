...
ServeBifly.post("/postProduto", async (req, res) => {
...
ServeBifly.post("/EditeProduct", async (req, res) => {
    try {
        ...
        const { id } = req.query; // id referente ao produto
        const { whatchInfor } = req.query; // qual a acao a ser feita
        const { information } = req.query; // novo dado da tabela
        switch (whatchInfor) {
...
ServeBifly.post('/addincart', async (req, res) => {
    const { product_id, emailUser } = req.body; //recebe o id do produto e o email do usuario para identificar a tabela(carrinho) do usuario que sera criada
...
ServeBifly.get('/getcart', async (req, res) => {
    const { email } = req.query; //pega o email na requisicao
    try {
        ...
        let arrwithitesn = []; // esse array armazena os itens
...
ServeBifly.post('/deletitencart', async (req, res) => {
    const { idDelete } = req.query; // pega o id do item(ele vem como string)
