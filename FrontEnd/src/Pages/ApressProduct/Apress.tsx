...
const Apress= ()=>{
    ...
    useEffect(()=>{
        setSrc(itensJson?.url_capa)
        console.log(replaceSrcImage)

        //busca descrição do livro
        const getDescription= async ()=>{
            const connect= async ()=>{
                        try {
                            const connectResponseBooks= await axios.get(`http://localhost:4000/getdescription`,
                                {
                                    params: {
                                        asin: itensJson.asin
                                    },
                            ...
                            console.log("dados retornado pela sistema: ", connectResponseBooks)
                        } catch (err) {
        ...
        if(itensJson.description == null || itensJson.description == undefined){
            getDescription()
        }else{
            setDescription(itensJson.description)
        }
    ...
    return(
        <section className="flex flex-col lg:flex-row gap-20 py-10 px-5 lg:px-0">
            ...
            <img className={`w-60 mx-auto lg:mx-10 lg:w-90 ${replaceSrcImage == '' ? 'flex' : 'hidden'}`} src={itensJson.url_capa} />
            <div className="flex flex-col gap-3 lg:mx-10">
                <h1 className="text-2xl w-130 font-semibold">{itensJson.titulo}</h1>
                <label className="flex flex-row gap-2">
                    <p className="text-[1em] font-semibold">Altor:</p>
                    <p className="text-[1em]">{itensJson.autor}</p>
                </label>
                <h2 className="text-[1.3em] font-semibold">description</h2>
                <div className="h-40 w-175 overflow-y-scroll">
                    <p>{description}</p>
                </div>
                <label className="flex justify-end flex-row gap-2">
                    <p className="text-[1em] font-semibold">Price:</p>
                    <p className="text-[2em] text-[#00ccbe]">{new Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL',}).format(itensJson.preco_desconto)}</p>
                </label>
